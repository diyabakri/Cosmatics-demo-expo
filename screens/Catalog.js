...
class Catalog extends Component{
        ...
        for(var i=0;i < collection.length;i++){
            ...
            for(var prop in curr_catagory){
                ...
                if(products_selected[curr_bar_code].quantity_selected>0 && products_selected[curr_bar_code].Checked){
                    ...
                    temp["selected"] =  products_selected[curr_bar_code].quantity_selected
                    Order.push(temp)
                }
            }
        }
        this.props.navigation.navigate("Order",{Order:Order, Client:this.state.Client})
    }
    ...
    Render_Product (doc,index){
        ...
        this.props.navigation.setOptions({title:"Order at: ₪"+this.state.Sum,headerBackTitle:""})
        ...
        //contains the current sum before the currrent change and all changes are done on exit before update
            ...
            <View>
                            ...
                            ()=>
                            {
                                ...
                                if(current_product.Checked){
                                    ...
                                    if(current_product.quantity_selected==0){
                                        if(doc.Quantity == 0){//if out of stock dont check 
                                            current_product.Checked = false; 
                                        }else{
                ...
                <View style = {current_product.Checked?styles.Checked_quantity_bar:styles.quantity_bar}>
                    ...
                    <TextInput 
                        ...
                        onChangeText = {text => { 
                            ...
                            else if(parseInt(text,10)>doc.Quantity){
                                text = doc.Quantity
                            }
                    ...
                    <Button transparent 
                    onPress={ ()=>{
                        if(current_product.quantity_selected<doc.Quantity){
                            if(current_product.Checked){
                                Sum+=parseInt(doc.Price,10)
                            }
                        current_product.quantity_selected++
                ...
                <View style ={styles.stock_style}>
                    <Text>{/* doc.Quantity contains the amount of the current product left in stock */}
                        In Stock: {doc.Quantity-current_product.quantity_selected}
                    </Text>
                </View>
        ...
        <View style = {styles.background_style}>
           <View style = {{flex:1,flexDirection:"row",flexWrap:"wrap",justifyContent:"center",width:"100%",height:"95%"}}>
               {this.state.loading?<ActivityIndicator  size="large" color="#6d6d6d" style = {{paddingTop:"75%",alignSelf:"center"}}/>: 
                ...
                {this.state.Sum>0?<TouchableOpacity onPress = {()=>this.Fin_order()} style = {styles.Fin_order}><Text adjustsFontSizeToFit= {true} style = {styles.Fin_Text_order}>Go to order checkout </Text></TouchableOpacity>:<View/>}
                </View>
               }  
           </View>
        </View>
